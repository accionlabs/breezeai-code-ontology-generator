...
const STATEMENT_TYPES = ["lexical_declaration", "variable_declaration", "public_field_definition", "if_statement", "for_statement", "for_each_statement", "switch_statement", "switch_expression", "return_statement", "enum_declaration"];
...
function isStringOrTemplateAssignment(node) {
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    if (vtype === "string_literal" || vtype === "interpolated_string_expression" || vtype === "verbatim_string_literal") return true;
  }
  return false;
}
...
function isCallAssignment(node) {
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    if (!value) continue;
    const vtype = value.type;
    if (vtype === "invocation_expression" || vtype === "await_expression" || vtype === "member_access_expression" || vtype === "object_creation_expression" || vtype === "lambda_expression" || vtype === "anonymous_method_expression") return true;
