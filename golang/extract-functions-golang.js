...
const STATEMENT_TYPES = ["lexical_declaration", "variable_declaration", "public_field_definition", "if_statement", "for_statement", "expression_switch_statement", "type_switch_statement", "return_statement"];
...
function isStringOrTemplateAssignment(node) {
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    if (vtype === "raw_string_literal" || vtype === "interpreted_string_literal") return true;
  }
  return false;
}
...
function isCallAssignment(node) {
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    if (!value) continue;
    const vtype = value.type;
    if (vtype === "call_expression" || vtype === "selector_expression" || vtype === "func_literal") return true;
  }
  return false;
}
