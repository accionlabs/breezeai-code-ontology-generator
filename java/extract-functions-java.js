...
const STATEMENT_TYPES = ["lexical_declaration", "variable_declaration", "public_field_definition", "if_statement", "for_statement", "enhanced_for_statement", "switch_expression", "return_statement", "enum_declaration"];
...
function isStringOrTemplateAssignment(node) {
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    if (vtype === "string_literal" || vtype === "text_block") return true;
  }
  return false;
}
...
function isCallAssignment(node) {
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    if (!value) continue;
    const vtype = value.type;
    if (vtype === "method_invocation" || vtype === "object_creation_expression" || vtype === "lambda_expression" || vtype === "method_reference") return true;
  }
  return false;
}
